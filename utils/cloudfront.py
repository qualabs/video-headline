...
def delete_distributions():
    ...
    for account in AWSAccount.objects.all():
        ...
        disabled_distributions = list(filter(
            lambda dist_entry: dist_entry['Status'] == 'Deployed' and not dist_entry['Enabled'],
            all_dist)
        )
...
def _delete_distribution(dist_id, aws_account_id):
    ...
    """
    cloudfront = get_cloudfront_client(AWSAccount.objects.get(account_id=aws_account_id))
...
@shared_task
def _delete_cloudfront_distribution(dist_id, aws_account_id, video_id, retry_countdown=300):
    """
    Steps for delete distribution:
        - get distribution configuration
        - find a ETag operation value
