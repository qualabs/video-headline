...
def get_cloudwatch_event(aws_account):
    if aws_account:
        events = boto3.client('events', aws_access_key_id=aws_account.access_key,
...
def put_rule(live):
    ...
    response = events.put_rule(Name=str(live.video_id),
                                EventPattern=json.dumps(conf),
                                State='ENABLED')
...
def put_targets(live):
    ...
    targets = [
        {
            'Id': str(live.id),
            'Arn': live.sns_topic_arn,
            'InputPath': '$.detail'
        },
    ]
    response = events.put_targets(Rule=str(live.video_id),
                                    Targets=targets)
...
def remove_targets(live):
    ...
    try:
        response = events.remove_targets(Rule=str(live.video_id),
                                        Ids=[str(live.id)])
    except events.exceptions.ResourceNotFoundException:
        raise EventsNotFoundException()