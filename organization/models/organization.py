...
class Organization(models.Model):
    ...
    contact_email = models.CharField(
        max_length=254, default='', blank=True, verbose_name='Contact Email'
    )
    bucket_name = models.CharField(
        max_length=100, editable=False, default='', verbose_name='Bucket'
    )
    cf_id = models.CharField(
        max_length=100, editable=False, default='', verbose_name='Cf_id'
    )
    cf_domain = models.CharField(
        max_length=100, editable=False, default='', verbose_name='Cf_domain'
    )
    ...
    upload_enabled = models.BooleanField(
        default=True, verbose_name='Enabled to upload videos'
    )
    traffic_enabled = models.BooleanField(
        default=True, verbose_name='Data traffic enabled'
    )
    security_enabled = models.BooleanField(
        default=False, verbose_name='URL security enabled'
    )
...
def org_pre_save_receiver(sender, instance, **kwargs):
    ...
    if (
        instance.security_enabled
        and not aws_account.cf_private_key
        and not aws_account.cf_key_pair_id
    ):
